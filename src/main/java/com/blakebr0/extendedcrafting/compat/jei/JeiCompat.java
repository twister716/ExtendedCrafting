...
public final class JeiCompat implements IModPlugin {
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        if (level != null) {
            ...
            if (ModConfigs.ENABLE_CRAFTING_CORE.get()) {
                registration.addRecipes(CombinationCraftingCategory.RECIPE_TYPE, manager.getAllRecipesFor(ModRecipeTypes.COMBINATION.get()).stream().map(RecipeHolder::value).toList());
            }

            if (ModConfigs.ENABLE_TABLES.get()) {
                var recipes = Stream.of(1, 2, 3, 4).collect(Collectors.toMap(tier -> tier, tier ->
                        RecipeHelper.getRecipes(ModRecipeTypes.TABLE.get())
                                .stream()
            ...
            if (ModConfigs.ENABLE_COMPRESSOR.get()) {
                registration.addRecipes(CompressorCraftingCategory.RECIPE_TYPE, manager.getAllRecipesFor(ModRecipeTypes.COMPRESSOR.get()).stream().map(RecipeHolder::value).toList());
            }

            if (ModConfigs.ENABLE_ENDER_CRAFTER.get()) {
                registration.addRecipes(EnderCrafterCategory.RECIPE_TYPE, manager.getAllRecipesFor(ModRecipeTypes.ENDER_CRAFTER.get()).stream().map(RecipeHolder::value).toList());
            }

            if (ModConfigs.ENABLE_FLUX_CRAFTER.get()) {
                registration.addRecipes(FluxCraftingCategory.RECIPE_TYPE, manager.getAllRecipesFor(ModRecipeTypes.FLUX_CRAFTER.get()).stream().map(RecipeHolder::value).toList());
            }
        }
    }
    ...
    public void registerRecipeTransferHandlers(IRecipeTransferRegistration registration) {
        if (ModConfigs.ENABLE_TABLES.get()) {
            registration.addRecipeTransferHandler(BasicTableContainer.class, ModMenuTypes.BASIC_TABLE.get(), BasicTableCategory.RECIPE_TYPE, 1, 9, 10, 36);
            registration.addRecipeTransferHandler(AdvancedTableContainer.class, ModMenuTypes.ADVANCED_TABLE.get(), AdvancedTableCategory.RECIPE_TYPE, 1, 25, 26, 36);
            registration.addRecipeTransferHandler(EliteTableContainer.class, ModMenuTypes.ELITE_TABLE.get(), EliteTableCategory.RECIPE_TYPE, 1, 49, 50, 36);
            registration.addRecipeTransferHandler(UltimateTableContainer.class, ModMenuTypes.ULTIMATE_TABLE.get(), UltimateTableCategory.RECIPE_TYPE, 1, 81, 82, 36);

            if (ModConfigs.TABLE_USE_VANILLA_RECIPES.get()) {
                registration.addRecipeTransferHandler(BasicTableContainer.class, ModMenuTypes.BASIC_TABLE.get(), mezz.jei.api.constants.RecipeTypes.CRAFTING, 1, 9, 10, 36);
            }

            if (ModConfigs.ENABLE_AUTO_TABLES.get()) {
                registration.addRecipeTransferHandler(BasicAutoTableContainer.class, ModMenuTypes.BASIC_AUTO_TABLE.get(), BasicTableCategory.RECIPE_TYPE, 1, 9, 11, 36);
                registration.addRecipeTransferHandler(AdvancedAutoTableContainer.class, ModMenuTypes.ADVANCED_AUTO_TABLE.get(), AdvancedTableCategory.RECIPE_TYPE, 1, 25, 27, 36);
                registration.addRecipeTransferHandler(EliteAutoTableContainer.class, ModMenuTypes.ELITE_AUTO_TABLE.get(), EliteTableCategory.RECIPE_TYPE, 1, 49, 51, 36);
                registration.addRecipeTransferHandler(UltimateAutoTableContainer.class, ModMenuTypes.ULTIMATE_AUTO_TABLE.get(), UltimateTableCategory.RECIPE_TYPE, 1, 81, 83, 36);

                if (ModConfigs.TABLE_USE_VANILLA_RECIPES.get()) {
                    registration.addRecipeTransferHandler(BasicAutoTableContainer.class, ModMenuTypes.BASIC_AUTO_TABLE.get(), mezz.jei.api.constants.RecipeTypes.CRAFTING, 1, 9, 11, 36);
                }
            }
        }

        if (ModConfigs.ENABLE_ENDER_CRAFTER.get()) {
            registration.addRecipeTransferHandler(EnderCrafterContainer.class, ModMenuTypes.ENDER_CRAFTER.get(), EnderCrafterCategory.RECIPE_TYPE, 1, 9, 10, 36);

            if (ModConfigs.ENABLE_AUTO_ENDER_CRAFTER.get()) {
                registration.addRecipeTransferHandler(AutoEnderCrafterContainer.class, ModMenuTypes.AUTO_ENDER_CRAFTER.get(), EnderCrafterCategory.RECIPE_TYPE, 1, 9, 10, 36);
            }
        }

        if (ModConfigs.ENABLE_FLUX_CRAFTER.get()) {
            registration.addRecipeTransferHandler(FluxCrafterContainer.class, ModMenuTypes.FLUX_CRAFTER.get(), FluxCraftingCategory.RECIPE_TYPE, 1, 9, 10, 36);

            if (ModConfigs.ENABLE_AUTO_FLUX_CRAFTER.get()) {
                registration.addRecipeTransferHandler(AutoFluxCrafterContainer.class, ModMenuTypes.AUTO_FLUX_CRAFTER.get(), FluxCraftingCategory.RECIPE_TYPE, 1, 9, 10, 36);
            }
        }
    }
    ...
    public void registerItemSubtypes(ISubtypeRegistration registration) {
        ...
        registration.registerSubtypeInterpreter(VanillaTypes.ITEM_STACK, ModItems.RECIPE_MAKER.get(),
                (stack, context) -> stack.getOrDefault(ModDataComponentTypes.RECIPE_MAKER, RecipeMakerComponent.EMPTY).type());
    }
}
